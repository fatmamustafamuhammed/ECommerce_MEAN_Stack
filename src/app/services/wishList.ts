...
export class wishListService {
  ...
  getWishLists(): Observable<ProductModel[]> {
    ...
    return this.http.get<any[]>(`${this.baseUrl}/customer/wishLists`).pipe(
      tap((response) => {
  ...
  deleteWishList(productId: string): Observable<any> {
    ...
    return this.http.delete<any>(`${this.baseUrl}/customer/wishLists/${productId}`).pipe(
      tap((response) => {
        console.log('Delete response:', response);
        this.wishLists.update((current) => current.filter((product) => product._id !== productId));
        this.loading.set(false);
      }),
      catchError((error) => {
  ...
  addToWishList(product: ProductModel): Observable<any> {
    ...
    return this.http.post<any>(`${this.baseUrl}/customer/wishLists/${productId}`, {}).pipe(
      tap(() => {
